...
#### 8.实现用户激活
- 8.1 配置邮件服务器和发件人sender:xxxxxxxxxxxx@qq.com
...
EMAIL_HOST_USER = 'xxxxxxxxxxxx@qq.com' # 授权的邮箱
EMAIL_HOST_PASSWORD = 'xxxxx' # 邮箱授权时获得的密码，非注册登录密码
EMAIL_FROM = '天天生鲜<xxxxxxxxxxxx@qq.com>' # 发件人抬头
...
#### 31 添加购物车购物车
- 31.1 实现添加购物车逻辑
    - 31.1.1 定义添加购物车视图类，配置url
    - 31.1.2 处理post请求，接收ajax的post发送过来的购物车数据，存储到reids
        - 获取购物车的参数，sku_id,count   request.POST.get()
        - 校验购物车参数
            - 判断是否缺少参数
                - 缺少参数返回code:2
            - 判断sku是否存在
                - 存在，获取商品SKU信息
                - 不存在返回code:3
            - 判断count是否合法 正则匹配整数数字
                - 不合法返回code:4
            - 判断是否超出库存
                - 超出库存返回code:5
        - 判断用户是否已登录
            - 已登录，获取user_id   request.user.id
                - 连接redis数据库
                - 获取对应user_id,sku_id的购物车数据
                - 判断是购物车数据否存在
                    - 存在，就对数量进行累加，不存在就赋新值（即不做处理）
                    - 再判断是否超出库存
                        - 超出库存返回code:5
                - 存储到reids数据库中 hset()
                - 查询购物车商品总数量
                    - 获取user_id对应的全部购物车数据（字典）
                    - 遍历取出count,进行累加
                - 响应结果，返回code:0,将数据展示在前端

            - 未登录，获取cookie中的购物车数据   request.COOKIES.get("cart")，注意是字符串类型的字典'{'sku_id1':1, 'sku_id2':2}'
                - 判断cookie是否存在
                    - 存在，进行反序列化操作，获得字典格式的JSON
                    - 不存在，定义空字典
                - 判断sku_id是否在cookie获得的字典中
                    - 存在，count累加
                    - 再判断是否超出库存
                        - 超出库存返回code:5
                - 存储sku_id和count到字典中
                ...
                - 响应结果，返回code:0,将数据展示在前端
                - 将更新后的字典序列化处理
                - 定义JsonResponse对象 返回code:0
                - 设置新cookie
                - 响应结果，将数据展示在前端

#### 32 购物车页面展示
- 32.1 实现展示购物车数据逻辑
    - 32.1.1 定义购物车数据视图类，配置url
    - 32.1.2 处理get请求，查询购物车数据，渲染模板
        - 判断用户是否登录
            - 已登录，获取user_id   request.user.id
                - 连接redis数据库
                - 获取user_id对应的所有购物车数据(字典)

            - 未登录， 获取cookie中的购物车数据   request.COOKIES.get("cart")
                - 判断cookie是否存在
                    - 存在，进行反序列化处理获得字典格式的JSON
                    - 不存在，定义空字典

            - 定义临时变量skus, total_count, total_sku_amount
            - 遍历购物车字典中的sku_id和count
                - 获取商品SKU信息
                    - 判断商品是否存在
                        - 不存在，continue继续下次循环
                - 计算这种商品总价
                - 动态给sku绑定count和amount,用于展示在前端
                - 添加sku到skus中
                - 计算累计金额和数量
            - 构造上下文，skus,total_count,total_sku_amount
            - 响应结果，渲染模板cart.html

#### 33 更新购物车
- 33.1 实现更新购物车逻辑
    - 33.1.1 定义更新购物车视图类，配置url
    - 33.1.2 处理post请求，实现手动输入
        - 获取购物车参数，sku_id,count
        - 进行参数校验
            - 判断是否缺少参数
                - 缺少参数返回code:2
            - 判断商品是否存在
        ...
        - 判断用户是否登录
            - 已登录， 获取user_id   request.user.id
                - 连接到redis数据库
                - 存储user_id，sku_id对应的购物车数量 更新购物车接口幂等设计
                - 响应结果，返回code:0
    ...
    - 34.1.2 处理post逻辑
        ...
        - 进行参数校验
            - 判断参数是否为空
                - 为空，返回code:2
            - 判断sku是否存在
                - 存在，获取sku_id对应商品SKU信息
                - 不存在，返回code:3
        - 判断用户是否登录
            - 已登录， 获取user_id   request.user.id
                - 连接到reids数据库
                - 删除数据库中对应user_id和sku_id的数量
